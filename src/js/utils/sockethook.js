...
export const useSession = ({ onClose, onHealthCheckFailed, onMessageReceived, onNotify, onOpen }) => {
  ...
  const connect = useCallback(
    async deviceId => {
      setSessionId();
      try {
        socketRef.current = new WebSocket(`wss://${window.location.host}${apiUrl.v1}/deviceconnect/devices/${deviceId}/connect`);
    ...
    [onSocketClose, onSocketOpen, onSocketError, onSocketMessage, onNotify]
  );
